...
async def rag_ingest_pdf(ctx: inngest.Context):
    ...
    def _upsert(chunks_and_src: RAGChunkAndSrc) -> RAGUpsertResult:
        ...
        logging.getLogger("uvicorn").info(f"Vector DB Upsert Time: {upsert_time:.4f}s for {len(chunks)} points")
        return RAGUpsertResult(ingested=len(chunks))

    chunks_and_src = await ctx.step.run("load-and-chunks", lambda: _load(ctx), output_type=RAGChunkAndSrc)
